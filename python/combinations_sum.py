...
class Solution:
	...
	def combinationSum2(self, candidates, target):
		...
		if size == 0:
			return output
		candidates.sort()
		self.make_combinations2(candidates, target, 0, list(), output)
		list_set = { tuple(a_list) for a_list in output }
		return [list(a_tuple) for a_tuple in list_set]

	def make_combinations2(self, candidates, target, start_index, a_list, output):
		if target == 0:
			output.append(a_list[:])
		else:
			size = len(candidates)
			for index in range(start_index, size):
				if target < candidates[index]:
					break
				a_list.append(candidates[index])
				self.make_combinations2(candidates, target-candidates[index], index + 1, a_list, output)
				a_list.pop()
