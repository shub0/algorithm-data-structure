...
class Solution(object):
    def findRightInterval(self, intervals):
        ...
        """
        import bisect
        tmp = [ [interval.start, index] for (index, interval) in enumerate(intervals) ]
        tmp.sort(key = lambda x: x[0])
        starts, idxes = zip(*tmp)
        output = list()
        size = len(intervals)
        for interval in intervals:
            index = bisect.bisect_left(starts, interval.end)
            if index == size:
                output.append(-1)
            else:
                output.append(idxes[index])
        return output
