...
class Solution:
    ...
    # @param end, a string
    # @param word_dict, a set of string
    # @return an integer
    # The following algorithm will work better for long word but small dict
    def ladderLength(self, start, end, word_dict):
        ...
        start_set.add(start)
        distance = 1
        while (end not in start_set) and (distance < len(word_dict)):
            new_candidate = set()
            for candidate in start_set:
                for word in word_dict:
                    if word in visited_word:
                        continue
                    if self.distance(word, candidate) == 1:
                        new_candidate.add(word)
                        visited_word.add(word)
            distance += 1
            start_set = new_candidate
        return distance if (end in start_set) else 0

    # @param start, a string
    # @param end, a string
    # @param dict, a set of string
    # @return an integer
    # The following algorithm will work better for short word but big dict
    def ladderLength2(self, start, end, word_dict):
        if len(start) != len(end):
            return 0
        visited_word = set()
        start_set = set()
        ...
        chars = ''.join([chr(x) for x in range(ord('a'),ord('z') + 1)])
        while distance < max_len:
            new_candidate = set()
            for candidate in start_set:
                for index in range(size):
                    for char in chars:
                        word = ''.join([candidate[:index],char,candidate[index+1:]])
                        if word == end:
                            return distance + 1
    ...
    # @param word_dict, a set of string
    # @return an integer
    def findLadders(self, start, end, word_dict):
        import collections
        word_dict.add(end)
        curr_level = {start}
        dp_set = collections.defaultdict(set)
        size = len(start)
        chars = ''.join([chr(x) for x in range(ord('a'),ord('z') + 1)])
        while curr_level and end not in dp_set:
            next_level = collections.defaultdict(set)
            for word in curr_level:
                for index in range(size):
                    for char in chars:
                        new_word = word[:index] + char + word[index+1:]
                        if new_word in word_dict and new_word not in dp_set:
                            next_level[new_word].add(word)
            curr_level = next_level
            dp_set.update(next_level)
        path =  [[end]]

        while path and path[0][0] != start:
            new_level_path = list()
            for current_path in path:
                head = current_path[0]
                new_level_path.extend( [ [word] + current_path for word in dp_set[head] ] )
            path = new_level_path
        return path
